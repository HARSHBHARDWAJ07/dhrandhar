...
export const Compare = ({
  ...
  const update = useCallback((clientX: number) => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      setSlider(percent); // ⚡ NO EASING → TRUE SYMMETRY
    });
  }, []);

  // ─── GLOBAL TRACKING (HOVER + DRAG) ─────────────────
  ...
  return (
    <div
      ref={ref}
      className={cn("relative h-105 w-full overflow-hidden", className)}
